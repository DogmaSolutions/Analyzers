...
The `[DSA023] Add comment for code review` action inserts a structured block comment above the flagged code. The comment includes the rule ID, enriched category, title with documentation link, and relevant external references. This action does not modify the code itself — it only adds a review annotation.

```cs
    /* [DSA023 / CySec + QA + Best Practice]: Use `Path.Combine` instead of string concatenation to build file system paths (see: https://github.com/DogmaSolutions/Analyzers/blob/main/docs/rules/DSA023.md)
     * CWE-22: Improper Limitation of a Pathname to a Restricted Directory ('Path Traversal') - https://cwe.mitre.org/data/definitions/22.html
     * `Path.Combine` documentation - https://docs.microsoft.com/en-us/dotnet/api/system.io.path.combine
     * IEC 62443 - FR 3 (System Integrity)
     */
