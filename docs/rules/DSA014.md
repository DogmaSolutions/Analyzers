...
The `[DSA014] Add comment for code review` action inserts a structured block comment above the flagged code. The comment includes the rule ID, enriched category, title with documentation link, and relevant external references. This action does not modify the code itself — it only adds a review annotation.

```cs
    /* [DSA014 / CySec + QA + Security]: Minimal API endpoints on route groups should have an explicit authorization configuration (see: https://github.com/DogmaSolutions/Analyzers/blob/main/docs/rules/DSA014.md)
     * MITRE, CWE-862: Missing Authorization - https://cwe.mitre.org/data/definitions/862.html
     * ASP.NET Core Minimal APIs - Authorization - https://learn.microsoft.com/en-us/aspnet/core/fundamentals/minimal-apis/security
     * OWASP: Broken Access Control - https://owasp.org/Top10/A01_2021-Broken_Access_Control/
