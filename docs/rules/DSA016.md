...
Repeated enumeration of a deferred `IEnumerable` backed by a database query or external data source can lead to non-deterministic behavior when the underlying data changes between evaluations. In security-sensitive code paths, this creates time-of-check to time-of-use (TOCTOU) exposure:

- An authorization check that calls `.Any(u => u.Id == userId)` to verify access and then calls it again to retrieve the record may get different results if the data was modified in between — the user could be authorized based on a record that no longer exists, or denied access to a record that was added after the check.
- An enumerable backed by a network stream or external API may yield different results on re-enumeration, causing one branch of the code to see data that another branch does not, leading to inconsistent state within the same request.

These inconsistencies can cascade into authorization bypasses, data integrity violations, or information disclosure — all because the developer assumed that calling the same method twice on the same source would return the same result.
...
```cs
    /* [DSA016 / CySec + QA + Code Smell]: Avoid repeated invocation of the same enumeration method with identical arguments (see: https://github.com/DogmaSolutions/Analyzers/blob/main/docs/rules/DSA016.md)
