...
The `[DSA028] Add comment for code review` action inserts a structured block comment above the flagged code. The comment includes the rule ID, enriched category, title with documentation link, and relevant external references. This action does not modify the code itself — it only adds a review annotation.

```cs
    /* [DSA028 / CySec + QA + Performance]: Prefer `ToArray` over `ToList` when return type is a read-only interface (see: https://github.com/DogmaSolutions/Analyzers/blob/main/docs/rules/DSA028.md)
     */
    return source.Where(o => o.IsActive).ToList();
```
