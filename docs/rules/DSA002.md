...
The `[DSA002] Add comment for code review` action inserts a structured block comment above the flagged code. The comment includes the rule ID, enriched category, title with documentation link, and relevant external references. This action does not modify the code itself — it only adds a review annotation.

```cs
    /* [DSA002 / CySec + QA + Design]: WebApi controllers should not contain data-manipulation business logics through a LINQ fluent query (see: https://github.com/DogmaSolutions/Analyzers/blob/main/docs/rules/DSA002.md)
     * MITRE, CWE-653: Improper Isolation or Compartmentalization - https://cwe.mitre.org/data/definitions/653.html
     * MITRE, CWE-1057: Data Access Operations Outside of Expected Data Manager Component - https://cwe.mitre.org/data/definitions/1057.html
